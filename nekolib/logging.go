...
var _inited = false
var logger = gologging.MustGetLogger("nekodb")
...
func InitLogger(debugMode bool) {
    ...
    if debugMode {
        _stdout.Color = true
        stdoutBackend.SetLevel(gologging.DEBUG, "nekodb")
    } else {
        stdoutBackend.SetLevel(gologging.INFO, "nekodb")
    }
    stderrBackend.SetLevel(gologging.ERROR, "nekodb")
